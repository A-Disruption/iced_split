...
// This crate depends on `iced` rather than `iced_core` so that a consumer's
// `[patch.crates-io] iced = ...` reaches it; `iced_core` is the same crate,
// re-exported, so the paths below are unchanged from upstream.
use iced::advanced::graphics::core as iced_core;
